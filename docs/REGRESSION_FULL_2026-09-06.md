...
## 0.6 高频主路径 · 新建/日常创建（★用户日常最常用,单独拎出必测）
> Home 首页 与 Cards 页 都有"新建"入口,两处都测。
### 新建目标(Home / Cards)
- [ ] 点「新建目标」卡 → 弹 compose 模态,色条=目标紫,tip 正确,textarea 自动聚焦
- [ ] 填目标内容 + 期限(date 选择器)+ 为什么(input)
- [ ] 空内容点「保存并联想历史」→ toast「写点内容吧」,不提交
- [ ] 有内容保存 → 按钮「保存中…」→ 成功 toast「已保存并联想历史」→ 模态关 → 卡片列表出现新卡
- [ ] 新目标卡:类型徽标、标题、meta(期限/为什么序列化正确)
- [ ] 目标卡「✓ 标记完成」→ done 样式;「↩ 恢复进行」;「✎ 编辑」改内容保存;删除 × 确认
- [ ] 保存后自动"联想历史":卡详情 related 有知识库相关内容
### 新建日记(Home / Cards)
- [ ] 点「新建日记」卡 → 弹模态,粉色,日记 tip,显示今日日期
- [ ] 7 个心情 chip(开心/平静/充实/疲惫/低落/焦虑/兴奋)→ 点选中(--mood-c 色)、再点取消
- [ ] 选心情 + 写内容 → 保存成功,日记卡头含「日期·心情X」+ 卡上心情小脸
- [ ] 空内容保存 → toast 拦截
### 新建笔记/任务(Cards 页)
- [ ] Cards 页类型分段(目标/日记/笔记/任务)切换,占位文案随类型变
- [ ] 新建笔记/任务 → 保存成功进列表
### 卡片通用操作
- [ ] 卡片列表点某卡 → 进详情(loading Thinking → 内容+related+卡内问答)
- [ ] 卡内问答:输入回车/发送 → AI 回答带来源;点来源→打开原文
- [ ] 删除卡 × → 确认框「确定删除?」→ 删除+toast;取消不删
- [ ] 未读卡右上红点;完成卡「✓」前缀

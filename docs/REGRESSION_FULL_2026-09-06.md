...
## 0. 待验证空白区（之前从没测过，重点补）
- [ ] **Apple Silicon（M 芯片）Mac**：客户端仅 x86_64 构建，M 上走 Rosetta，从未验证。借一台 M 机跑：启动+入库+问答+OCR+微信同步。
- [ ] **HD 真实微信图 OCR 出结果耗时**：mac2 计时（进行中）。
- [ ] **AI 欠费提示**：用欠费/额度用尽 key（或临时触发 402）实测。
- [ ] **微信助手新向导**：三版（Win/Mac Intel/Mac arm64）步骤条+已有钥匙确认+修跳步 真机走查。
- [ ] **死代码确认**：Relationships 的 openBrief/deepen/reportContact 当前无 JSX 入口——确认是否补入口或删除。

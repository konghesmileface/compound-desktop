...
export const SOURCES = {
  ...
  ios: {
    ...
    steps: [
      ...
      { t: '第三步 · 在助手里点「开始导入(连手机)」', d: '助手会自动:备份手机 → 找到微信明文库 → 抽成「[时间] 我/对方: 内容」的对话 → 逐个入库。界面有五段式进度(连接手机 → 备份聊天 → 解析数据 → 识别联系人 → 写入大脑),你只需等它跑完。中途若因手机锁屏/供电不足断开,会自动断点续传、不用从头。' },
    ...
    faq: [
      ...
      { q: '备份好慢 / 老是中断?', a: '第一次全量备份和你微信历史多少有关,聊天多可能十几分钟。★如果老是中断,多半是供电不足(整机备份耗电大、普通 USB 口只有 500mA 充不过耗、电量越掉越容易断):建议先把 iPhone 充到 80% 以上再开(充电时先别开备份),并开飞行模式 + 屏幕亮度调最低省电,数据线直插电脑机身口(别用扩展坞)。断了会自动断点续传,不会白干。' },
